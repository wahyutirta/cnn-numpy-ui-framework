...
class LENET5:
    ...
    def lenet_train(self, **params):
        ...
        method = params.get("method", "rmsprop")            # Default
        ...
        for ep in range(epochs):
            ...
            if not hasattr(self, 'optimizer'):
                if (method == "gd_momentum"):
                    self.optimizer = Optimizer_SGD(learning_rate=alpha, decay=0.0, momentum=0.9)
                elif (method == "adam"):
    ...
    def predict(self, X, Y):
        ...
        if len(Y.shape) == 2:
            ...
            temp_predictions = np.argmax(predictions, axis=1)
            accuracy = np.sum(np.equal(temp_y,temp_predictions))/len(temp_y)
        print("predictions stats ::: Loss: ", loss, "weight_sum: ", weight_sum, "acc ::", accuracy)
...
def main():
    ...
    Xtest, Ytest = spiral.create_data(samples=100, classes=3)
    N = len(Ytest)
    Y_test = np.zeros((N, 3))
    Y_test[np.arange(N), Ytest[range(0, N)]] = 1
    lenet5.predict(Xtest, Y_test)
