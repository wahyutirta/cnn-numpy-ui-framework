...
class LENET5:
    ...
    @staticmethod
    def update_parameters(layers, optimizer):
        """
    ...
    def lenet_train(self, **params):
        ...
        self.learningRate  = params.get("learningRate", 0.01)            # Default 0.1
        ...
        print("Training on params: batch=", self.batch, " learning rate=", self.learningRate, " L2 regularization=", zeta, " method=", method, " epochs=", epochs)
        ...
        for ep in range(epochs):
            ...
            for x, y in X_batches:
                ...
                LENET5.update_parameters(self.layers, self.optimizer)
    ...
    def save_parameters(self,mainPath):
        ...
        for layer in self.layers:
            if isinstance(layer, CONV_LAYER):
                ...
                np.savez(str(path) + "/" + "jepun-" + layer.layer_name + "-" + str(self.method) + str(self.epochs) + str(self.batch) + str(self.learningRate) , layer.kernel, layer.bias)
        ...
        np.savez(str(path) + "/" + "jepun-"  + "valid_history" +"-" + str(self.method) + str(self.epochs), self.valid_loss_history, self.valid_acc_history,self.valid_steps )
        np.savez(str(path) + "/" + "jepun-"  + "step_history" +"-" + str(self.method) + str(self.epochs), self.loss_history, self.acc_history,self.weights_history, self.n_steps )
        np.savez(str(path) + "/" + "jepun-"  + "epoch_history" +"-" + str(self.method) + str(self.epochs), self.epoch_loss, self.epoch_acc, self.epoch_weight,self.epochs )
    ...
    def load_train_details(self, **params):
        ...
        fname = "jepun-" + "valid_history" +"-" + str(method) + str(epochs) + ".npz"
        ...
        fname = "jepun-" + "step_history" +"-" + str(method) + str(epochs) + ".npz"
        path = os.path.join(mainPath,str(method),fname)
        arr_files = np.load(path)
        ...
        fname = "jepun-" + "epoch_history" +"-" + str(method) + str(epochs) + ".npz"
        path = os.path.join(mainPath,str(method),fname)
        arr_files = np.load(path)
    ...
    def load_parameters(self, **params):
        ...
        learningRate = params.get("learningRate", 0.01)
        mainPath = params.get("mainPath", "")
        ...
        for layer in tqdm(self.layers, desc="loading layers..."):
            if isinstance(layer, CONV_LAYER):
                ...
                fname = "jepun-" + layer.layer_name + "-" + str(method) + str(epochs) + str(batch) + str(learningRate) + ".npz"
                path = os.path.join(mainPath,str(method),fname)
...
def main():
    ...
    start = timeit.default_timer()
    mylenet.lenet_train(method=method, epochs=epochs, batch=batch, learningRate=lr, zeta=0)
    stop = timeit.default_timer()
    ...
    mylenet.load_parameters(mainPath=mainPath,epochs=epochs,method=method, learningRate=lr, batch=batch )
    mylenet.lenet_predictions(X_test, Y_test)
    
    """ load training history """
    mylenet.load_train_details(mainPath=mainPath,epochs=epochs,method=method, batch=batch, learningRate=lr)
