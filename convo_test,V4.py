    ...
    def save_parameters(self):
        ...
        for layer in self.layers:
            if isinstance(layer, CONV_LAYER):
                np.savez("conv" + str(self.layers.index(layer)) + "-" + str(self.method), layer.kernel, layer.bias)
            elif isinstance(layer, FC_LAYER):
                np.savez("fc" + str(self.layers.index(layer)) + "-" + str(self.method), layer.kernel, layer.bias)
        pass
    ...
    def load_parameters(self):
        ...
        for layer in self.layers:
            if isinstance(layer, CONV_LAYER):
                ...
                path = os.path.join(self.method,fname)
                print(path)
                
            elif isinstance(layer, FC_LAYER):
                fname = "fc" + str(self.layers.index(layer)) + "-" + str(self.method) + ".npz"
                path = os.path.join(self.method,fname)
                print(path)
...
def main():
    ...
    mylenet = LENET5(X_train, Y_train, X_test, Y_test, method="adam")
    
    start = timeit.default_timer()
    #mylenet.lenet_train(method="gd_momentum", epochs=500, batch=32, alpha=0.01, zeta=0)
    stop = timeit.default_timer()
