...
class LENET5:
    ...
    def __init__(self, **params):
        ...
        self.Y = params.get("t_output",[[]] )
        self.Xv = params.get("v_input",[[[[]]]] )
        self.Yv = params.get("v_output",[[]] )
        self.method = params.get("method","adam")
        self.epochs = params.get("epochs", 201)
        self.batch = params.get("batch", 32)
        self.learningRate = params.get("learningRate", 0.0001)
        self.name = params.get("name", "jepun")
        self.modelName = str("-" + str(self.name) + str(self.learningRate) + str(self.batch) + str(self.method) + str(self.epochs))
    ...
    def save_parameters(self,mainPath):
        ...
        for layer in self.layers:
            if isinstance(layer, CONV_LAYER):
                ...
                np.savez(str(path) + "/" + layer.layer_name + self.modelName, layer.kernel, layer.bias)
        ...
        np.savez(str(path) + "/" + "valid_history" + self.modelName, self.valid_loss_history, self.valid_acc_history,self.valid_steps )
        np.savez(str(path) + "/" + "step_history" + self.modelName, self.loss_history, self.acc_history,self.weights_history, self.n_steps )
        np.savez(str(path) + "/" + "epoch_history" + self.modelName, self.epoch_loss, self.epoch_acc, self.epoch_weight,self.epochs )
...
def main():
    ...
    if mode == "train":
        mylenet = LENET5( name = name ,t_input = X_train, t_output = Y_train, v_input = X_test, v_output = Y_test, method = method, epochs = epochs, batch = batch, learningRate = learningRate) 
        ...
        del X_train, Y_train
        mylenet = LENET5( name= name, v_input = X_test, v_output = Y_test, method = method, epochs = epochs, batch = batch, learningRate = learningRate) 
